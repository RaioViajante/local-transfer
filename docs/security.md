...
Durable trusted-peer recognition is persisted as one versioned, bounded `trusted-peers` file in the same configuration directory, with a persistent sibling `trusted-peers.lock` file; both receive the same `0600`/`0700` treatment on Unix and the same inherited-ACL limitation on Windows. Each record binds only an authenticated key identity (opaque scheme discriminator + 1–64 fingerprint bytes) and an optional presentation-only label. No hostname, address, endpoint, discovery key, pairing-attempt identifier, or transcript is a field. The label is stored as canonical lowercase hexadecimal purely for deterministic parsing — this is a reversible encoding, not encryption: anyone able to read the file recovers the label. Records are neither encrypted at rest nor hardware-backed; the fingerprint bytes are public key-identity material, not secrets, and structural persistence never makes bytes authenticated — a stored token only matters once the future authenticated-pairing layer re-establishes authentication and compares.

Every mutation runs as one load-modify-atomic-replace transaction serialized across processes by an exclusive OS advisory lock on `trusted-peers.lock` (non-blocking: a lock held elsewhere is a typed "busy" error, not a wait), so concurrent writers cannot lose an update or resurrect a removed record. Reads are lock-free and rely on atomic replacement plus reading through a pinned open file descriptor. On Unix the store file is replaced atomically and its directory entry fsynced; on Windows the atomic rename and file flush apply but no directory fsync. The guarantee is flushed contents plus atomic rename — not power-loss or transactional-filesystem consistency. A loaded file is untrusted local input: the read has a hard byte ceiling before any allocation, then it is version-checked and fully validated before any record is used, and any corruption fails the whole load closed rather than yielding a partial or fallback identity. A symbolic link at the store or lock path is refused, and its target is never read, written, or `chmod`ed; this defends the ordinary case but is not resistance to a same-user process racing every filesystem operation. Cross-compilation confirms the code builds for Windows but is not a runtime check of Windows filesystem semantics.
