...
`trusted_peers::TrustedPeerStore` is the durable side of the trusted-peer model in [trust.md](trust.md). It keeps one versioned, size-bounded `trusted-peers` file (plus a persistent sibling `trusted-peers.lock`) in the same configuration directory, holding at most 128 records; each record binds an authenticated key identity (opaque `u16` scheme discriminator plus 1–64 fingerprint bytes — a persisted recognition token, not proof of authentication) to an optional presentation-only label. Nothing transient — discovery keys, addresses, endpoints, hostnames, pairing-attempt identifiers, transcripts — is stored. Every mutation runs as one exclusively locked load-modify-atomic-replace transaction (an OS advisory lock on the `.lock` file serialises writers across processes and is non-blocking with a typed busy outcome; synced temporary file, rename, and on Unix a directory fsync), reusing the identity stores' restrictive-permission handling. Reads are lock-free and bounded by a hard allocation ceiling; loading returns only fully validated records; corruption fails the whole load closed; a symlinked store or lock path is refused. `load` and `remove` are public; a record can only be constructed in-crate by the future verified-pairing transition, so no adapter can turn advisory data into trust. Effective runtime-trust orchestration (disabling trust immediately on reset/revocation) sits above this store and is a separate concern.
